  ...
  <el-card class="trademark_container" shadow="hover">
    <template #header>
      <div class="card-header">
        <el-button class="button" icon="CirclePlus" @click="dialogVisible = true">
          新增品牌
        </el-button>
      </div>
    </template>
    <el-table :data="tableData" stripe border style="width: 100%">
      ...
      <el-table-column align="center" label="品牌LOGO">
        ...
        <template #="{ row }">
          <el-image style="width: 100px; height: 100px" :src="row.logoUrl" preview-teleported
      ...
      <el-table-column align="center" prop="" label="操作">
        ...
        <template #="{ row }">
          ...
          <el-popconfirm :title="`确认要删除品牌 [${row.tmName}] 吗`" @confirm="deleteItem(row.id)">
            <template #reference>
    ...
    <template #footer>
      <el-pagination v-model:currentPage="pages.page" v-model:pageSize="pages.limit" :page-sizes="[3, 5, 10, 20]"
        layout="prev,pager,next,jumper,->,total,sizes" :total="listTotal" background
        @size-change="pagesChange($event, 'limit')" @current-change="pagesChange($event, 'page')" />
    </template>
    <!-- 对话框 -->
    <el-dialog align-center v-model="dialogVisible" :title="`品牌${fromdata.id ? '编辑' : '新增'}`" width="500"
      :before-close="handleClose">
      <el-form label-position="left" :rules="rules" label-width="auto" ref="ruleFormRef" :model="fromdata"
        ...
        <el-form-item label="品牌LOGO" prop="logoUrl">
          <el-upload class="avatar-uploader" action="/dev-api/admin/product/fileUpload" :show-file-list="false"
            ...
            <el-image v-if="fromdata.logoUrl" :src="fromdata.logoUrl" class="avatar" />
...
// 之前有多少页
const beforeRowNum = computed(() => (reqpPages.value.page - 1) * reqpPages.value.limit);
// 获取列表
const getList = async () => {
...
const deleteItem = (id: string) => {
  reqTrademarkDelete({ id }).then((_res: any) => {
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid) => {
    if (valid) {
      reqTrademarkSave(fromdata).then((_res) => {
